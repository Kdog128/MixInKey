...
export function CamelotWheel({ keyA, keyB, className, disabled, comingSoonNote }: CamelotWheelProps) {
  const cx = 200;
  const cy = 200;
  // Outer B ring (major)
  const outerB = 168;
  ...
  return (
    <div className={cn("flex w-full max-w-full flex-col items-center gap-3", className)}>
      ...
      {disabled && comingSoonNote && (
        <div className="absolute inset-0 flex items-center justify-center px-4">
          <p className="text-xs font-medium text-center text-muted-foreground bg-background/80 backdrop-blur-sm rounded-lg px-3 py-2 border border-border/60">
            {comingSoonNote}
          </p>
        </div>
      )}
